// src/config/projectHierarchyConfig.ts
export const PROJECT_HIERARCHY_CONFIG = {
  ...
  validation_rules: [
    {
      code: "L3_REQUIRED_WHEN_MULTIPLE",
      rule: "If project has more than 1 Level-3 node, Level-3 selection is mandatory for inventory/lead records.",
    },
    {
      code: "L4_OPTIONAL_DEFAULT",
      rule: "Level-4 is optional by default; make it required only when structure demands it (e.g., COMMERCIAL:FLOOR_WISE).",
    },
    {
      code: "UNIQUE_L3_PER_PROJECT",
      rule: "Level-3 names must be unique within a project.",
    },
    {
      code: "UNIQUE_L4_PER_L3",
      rule: "Level-4 names must be unique within the same Level-3 parent.",
    },
    {
      code: "NO_INVENTORY_WITHOUT_HIERARCHY",
      rule: "Prevent inventory creation if required hierarchy nodes are missing.",
    },
  ],
} as const;
...
const PROJECT_TYPE_CODES = PROJECT_HIERARCHY_CONFIG.level1_project_types.map(
  (t) => t.code,
);

/** Narrow API/string codes to ProjectType when valid */
export const parseProjectType = (
  code: string | null | undefined,
): ProjectType | null => {
  const normalized = code?.trim().toUpperCase();
  if (!normalized || !PROJECT_TYPE_CODES.includes(normalized)) {
    return null;
  }
...
export interface HierarchyConfig {
  ...
  inventory_unit_types: string[];
  inventory_extra_fields?: Array<{
    code: string;
    name: string;
    values: string[];
  }>;
...
export const getHierarchyConfig = (
  type: ProjectType,
  structure: string,
): HierarchyConfig | null => {
  return (
    PROJECT_HIERARCHY_CONFIG.level3_level4_hierarchy_by_structure[type]?.[
      structure
    ] || null
  );
};

export const getAllowedUnitTypes = (
  type: ProjectType,
  structure: string,
): string[] => {
  return getHierarchyConfig(type, structure)?.inventory_unit_types || [];
};

export const getStructuresForType = (type: ProjectType) => {
  return PROJECT_HIERARCHY_CONFIG.level2_project_structures[type] || [];
};
