...
import { projectHasInventory } from "../utils/projectInventoryPresence.js";

const getProjectIdFromParams = (req) => req.params.projectId || req.params.id;

const fetchProjectRow = async (client, projectId) => {
  const result = await client.query(
    `SELECT id, name, project_type, project_structure, inventory_subcategory
     FROM projects
  ...
  try {
    ...
    if (columnDiagnostics.migration_required) {
      return res.status(503).json({
        ...
        details:
          "Run migration/2026-03-24-project-units.sql to add project_type and project_structure columns.",
    ...
    const hasInventory = await projectHasInventory(client, projectId);
    ...
    res.json({
      ...
      storage: {
        level1Field: "projects.project_type",
        level2Field: "projects.project_structure",
        hierarchyTable: "project_hierarchy_nodes",
        unitsTable: "project_units",
        configFile: "api/config/projectConfig.js",
      },
    });
  } catch (error) {
...
export const saveProjectInitialSetup = async (req, res) => {
  ...
  const {
    ...
    inventory_subcategory: inventorySubcategory,
  } = req.body;
  ...
  try {
    ...
    if (inventorySubcategory !== undefined) {
      updates.push(`inventory_subcategory = $${index++}`);
      values.push(inventorySubcategory?.trim() || null);
    }
    ...
    const updateResult = await client.query(
      `UPDATE projects SET ${updates.join(", ")}
       WHERE id = $${index}
       RETURNING id, name, project_type, project_structure, inventory_subcategory`,
      values,
    );
    ...
    const unitCount = await countUnits(client, projectId);
    const hasInventory = await projectHasInventory(client, projectId);
    const status = buildProjectSetupStatus(saved, {
      level3NodeCount,
      unitCount,
      hasInventory,
    });
...
/** Hard-delete all layout inventory for a project (units, towers, floors, hierarchy, unit types). */
...
export const getProjectHasInventory = async (req, res) => {
  const projectId = getProjectIdFromParams(req);
  if (!projectId) {
    return res.status(400).json({ error: "Project ID is required" });
  }
  ...
  try {
    ...
    const hasInventory = await projectHasInventory(client, projectId);
    res.json({ data: { hasInventory } });
  } catch (error) {
    console.error("getProjectHasInventory error:", error);
    res.status(500).json({ error: "Failed to check project inventory" });
  } finally {
    if (client) client.release();
  }
};
